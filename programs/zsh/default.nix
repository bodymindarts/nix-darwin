{config}: {
  ...
  shellAliases = {
    ...
    nix-update = "export NIXPKGS_ALLOW_UNFREE=1; pushd ~/.config/nix-darwin; nix flake update --impure; sudo sh -c 'NIXPKGS_ALLOW_UNFREE=1 darwin-rebuild switch --flake ~/.config/nix-darwin --impure'; source ~/.zshrc; popd";
  };
}
